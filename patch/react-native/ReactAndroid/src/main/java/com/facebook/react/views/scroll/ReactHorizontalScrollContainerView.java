...
public class ReactHorizontalScrollContainerView extends ReactViewGroup {
  ...
  protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
    if (mLayoutDirection == LAYOUT_DIRECTION_RTL) {
      if (left < 0) {
        // When the layout direction is RTL, and the width of the ScrollView exceeds screen width,
        // we expect Yoga to give us a layout that extends off the screen to the left (left < 0),
        // so we re-center it with left=0
        int newLeft = 0;
        int width = right - left;
        int newRight = newLeft + width;
        Log.i("JW", "onLayout, width: " + width + ", prev: " + mCurrentWidth + ", right: " + right + ", left: " + left + ", newLeft: " + newLeft + ", newRight: " + newRight);
        setLeft(newLeft);
        setRight(newRight);
      }
...
//        if (offsetX > 0) {
          Log.i("JW", "scrollTo " + offsetX + ", parentX: " + parent.getScrollX() + " mCurrentWidth: " + mCurrentWidth + ", getWidth(): " + getWidth() + ", parent: " + parent.getWidth());
